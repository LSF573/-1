...
<!-- 品质联盟 -->
  <div>
    <template v-for="(item,index) in CompanyData">
      <template v-if="item.restaurant.support_tags.length>1">
        <!-- <template v-for="(result,i) in item.restaurant.support_tags[0]"> -->
...
export default {
  ...
  created(){
    ...
    //     // console.log(element)
    //     //   console.log(element.restaurant.support_tags.length)
    //    for(let i=0;i<element.restaurant.support_tags.length;i++){
    //     if(i>1){
    //         if(element.support_tags[i].text=='品质联盟'){
